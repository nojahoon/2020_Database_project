...
<!-- 음악등록  -->
	...
	<% 
	   ...
	   String getMuDate=request.getParameter("muDate");   //음악 앨범
	   //String getMuDate=request.getParameter("date");     //개봉날짜
	  ...
	  <%   return;
	   ...
	   try {
			...
			String query = "select count(*) as cnt from source_basic where s_title='" +getMuName+"'";
			//DB Connection 생성
			...
			else
			{
				...
				try{
					...
					query="INSERT INTO Source_Basic(s_type,s_title) VALUES('"+getStype+"' , '"+getMuName+"')";
					stmt =conn.createStatement();
					...
					query="select * from Source_Basic where s_title='"+getMuName+"'";
					stmt=conn.createStatement();
					result=stmt.executeQuery(query);
					result.next();
					...
					//Source_Music Table에 삽입
					query="INSERT INTO Source_Music(mu_singer,mu_genre,mu_Date,source_id) VALUES('"+getMuSinger+"' , '"+getMuGenre+"' , '" +getMuDate+"','"+getSId+"')";
					stmt =conn.createStatement();
					stmt.executeUpdate(query);
